...
class ROMA:
    ...
    def bulk_fix_pc_sign(self, GeneScore, SampleScore, Wei=None, Mode='none', DefWei=1,
    ...
    def bulk_orient_pc1(self, pc1, X, raw_X_subset, gene_set_name=None):
        """
    ...
    def several_devices_randomset_parallel_gpu(self, key, iters=100):
        """
        GPU-parallel computation of null distributions via JAX vectorization,
        adapted to use multiple GPUs via jax.pmap.
        
        Instead of converting the entire self.adata.X (a scipy sparse matrix)
        to dense, we use a JAX pure callback to extract dense submatrices for each
        random subset (of size nullgenesetsize). The work is then partitioned across
        all available GPUs.
        
        Returns:
        Tuple (expl_ratios, med_exps, projections_all) with shapes:
            expl_ratios: (iters,)
            med_exps: (iters,)
            projections_all: (iters, n_samples)
        """
        import jax
        import jax.numpy as jnp
        import jax.scipy.linalg as jsp_linalg
        ...
        # self.adata.X is a scipy sparse matrix of shape (n_samples, n_genes)
        n_samples, n_genes = self.adata.X.shape
        nullsize = self.nullgenesetsize  # number of genes to sample per iteration
        ...
        # Step 2: Define a pure Python function to extract a dense submatrix from the sparse matrix.
        def get_dense_subset(indices):
            # indices: a numpy array (concrete) of shape (nullsize,)
            # Extract columns from self.adata.X which is (n_samples, n_genes)
            return self.adata.X[:, indices].toarray()  # (n_samples, nullsize)

        expected_shape = (n_samples, nullsize)

        # Step 3: Define a per-iteration function.
        def one_iteration(indices):
            # indices is a 1D array of concrete gene indices.
            # Use a pure callback to get the dense submatrix.
            X_subset = jax.pure_callback(
                get_dense_subset,
                jnp.zeros(expected_shape, dtype=jnp.float64),
                indices
            )
            # Now X_subset is a dense JAX array with shape (n_samples, nullsize).
            # Compute full SVD on X_subset.
            U, s, Vh = jsp_linalg.svd(X_subset, full_matrices=False)
            # Compute transformed data: X_transformed = X_subset @ Vh.T.
            X_transformed = X_subset @ Vh.T  # shape: (n_samples, k)
            # Compute explained variance per component.
            exp_var = jnp.var(X_transformed, axis=0)
            # Compute full variance of X_subset.
            full_var = jnp.var(X_subset, axis=0).sum()
            # L1 explained variance ratio is defined as exp_var[0] / full_var.
            expl_ratio = exp_var[0] / full_var
            # For PC1 projections, we use the first row of Vh.
            pc1 = Vh[0, :]  # shape: (nullsize,)
            projections = X_subset @ pc1  # shape: (n_samples,)
            med_exp = jnp.median(projections)
            return expl_ratio, med_exp, projections

        # Step 4: Check how many devices (GPUs) are available.
        devices = jax.devices()
        num_devices = len(devices)
        if num_devices > 1:
            # Partition batched_indices into shape (num_devices, local_iters, nullsize)
            local_iters = iters // num_devices
            # Truncate if necessary:
            total_iters = num_devices * local_iters
            batched_indices = batched_indices[:total_iters]
            batched_indices = batched_indices.reshape(num_devices, local_iters, nullsize)
            
            # For each device, we map the one_iteration function over its local batch.
            # We use pmap to parallelize across devices, and vmap to process each device's batch.
            @jax.pmap
            def pmap_one_iteration(batch_indices):
                return jax.vmap(one_iteration)(batch_indices)
            
            expl_ratios, med_exps, projections_all = pmap_one_iteration(batched_indices)
            # Flatten the device and batch dimensions.
            expl_ratios = expl_ratios.reshape(-1)
            med_exps = med_exps.reshape(-1)
            # projections_all: shape (num_devices, local_iters, n_samples) -> (total_iters, n_samples)
            projections_all = projections_all.reshape(-1, n_samples)
        else:
            # Single GPU: just use vmap.
            expl_ratios, med_exps, projections_all = jax.vmap(one_iteration)(batched_indices)

        # Optionally store the computed null distributions.
        self.nulll1 = expl_ratios
        self.null_median_exp = med_exps
        self.null_projections = projections_all
    ...
    def batched_randomset_parallel_gpu(self, key, total_iters=1000, batch_size=50):
        """
        GPU-parallel computation of null distributions using incremental loading,
        pure callbacks, and sharding across devices via pjit.
        
        This function uses self.adata.X (a scipy sparse matrix of shape (n_samples, n_genes)).
        For each iteration:
        1. Generate a set of gene indices (of length nullsize).
        2. Use a pure callback to extract the dense submatrix corresponding to these indices from self.adata.X.
        3. Compute full SVD (using jax.scipy.linalg.svd) on the dense subset.
        4. Compute the transformed data: X_transformed = X_subset @ Vh.T.
        5. Compute the explained variance ratio as: 
                expl_ratio = variance(X_transformed[:,0]) / sum(variance(X_subset, axis=0))
        6. Compute PC1 projections and its median.
        
        The computation is sharded across devices using pjit.
        """
        ...
        def get_dense_subset(adata, indices):
            """
            Given a sparse matrix adata.X and a concrete numpy array 'indices'
            (of shape (nullsize,)), extract the corresponding columns and convert to dense.
            """
            return adata.X[:, indices].toarray()

        # Get shape from the sparse matrix.
        # Assume self.adata.X is a scipy sparse matrix with shape (n_samples, n_genes)
        n_samples, n_genes = self.adata.X.shape  
        nullsize = self.nullgenesetsize  # number of genes to sample per iteration
        ...
        def gen_indices(key):
            perm = jax.random.permutation(key, n_genes)
            return perm[:nullsize]
        keys = jax.random.split(key, total_iters)
        all_indices = jax.vmap(gen_indices)(keys)  # shape: (total_iters, nullsize)

        # Partition all_indices into batches of size batch_size.
        num_batches = total_iters // batch_size
        total_iters = num_batches * batch_size  # truncate if necessary
        all_indices = all_indices[:total_iters].reshape(num_batches, batch_size, nullsize)

        # Step 2: Expected shape for each dense submatrix.
        expected_shape = (n_samples, nullsize)

        # Step 3: Define the per-iteration function.
        def one_iteration(indices):
            # 'indices' is a 1D jnp array (length nullsize) of concrete gene indices.
            X_subset = jax.pure_callback(
                lambda inds: get_dense_subset(self.adata, inds),
                jnp.zeros(expected_shape, dtype=jnp.float64),
                indices
            )
            X_subset = jnp.array(X_subset, dtype=jnp.float64)
            # Compute full SVD on X_subset.
            U, s, Vh = jsp_linalg.svd(X_subset, full_matrices=False)
            # Compute transformed data: X_transformed = X_subset @ Vh.T.
            X_transformed = X_subset @ Vh.T  # shape: (n_samples, k)
        ...
        # Step 4: Set up device sharding via pjit.
        # Determine available devices.
        devices = np.array(jax.devices())
        num_devices = len(devices)
        # Partition the batches among devices.
        local_batches = num_batches // num_devices
        total_batches = num_devices * local_batches
        batched_indices = all_indices[:total_batches].reshape(num_devices, local_batches, batch_size, nullsize)

        # Define a function to process one batch.
        def process_batch(batch_indices):
            # batch_indices has shape (batch_size, nullsize)
            return jax.vmap(one_iteration)(batch_indices)
        
        # Create a pjit version of process_batch.
        process_batch_pjit = pjit.pjit(
            process_batch,
            in_shardings=P('batch', None),
            out_shardings=(P('batch', None), P('batch', None), P('batch', None))
        )
        
        # Create a device mesh over the available GPUs.
        with Mesh(devices, ('device',)):
            batched_results = jax.pmap(process_batch_pjit)(batched_indices)
            # batched_results is a tuple of arrays with shape (num_devices, local_batches, batch_size, ...)
            expl_ratios = np.concatenate(np.array(batched_results[0]), axis=0)
            med_exps = np.concatenate(np.array(batched_results[1]), axis=0)
            projections = np.concatenate(np.array(batched_results[2]), axis=0)

        # Store the results in the object.
        self.nulll1 = expl_ratios  # shape: (total_iters,)
        self.null_median_exp = med_exps  # shape: (total_iters,)
        self.null_projections = projections  # shape: (total_iters, n_samples)
        
        return expl_ratios, med_exps, projections


    def ol_batched_randomset_parallel_gpu(self, key, total_iters=100, batch_size=100):
        """
    ...
    def compute(self, selected_gene_sets, parallel=False, incremental=False, partial_fit=False, algorithm='randomized', loocv_on=True, double_mean_centering=False, batch_size=100):
        #pl.adata = self.adata
        """
        Computes ROMA
        ...
        for gene_set_name in sorted_gene_sets:
            ...
            if parallel:
                if self.computation_mode == 'bulk':
                    ...
                    self.nulll1, self.null_median_exp, self.null_projections = self.batched_randomset_parallel_gpu(key, total_iters=iters, batch_size=batch_size)
